...
rm -rf model_record
timeout 120 python ../main.py train small_Si_mixed.json 2>&1 | tee train_small_Si_mixed_v6.log
rm -rf model_record
timeout 120 python ../main.py train medium_C_mixed.json 2>&1 | tee train_medium_C_mixed_v6.log
rm -rf model_record
timeout 300 python ../main.py train large_water_mixed.json 2>&1 | tee train_large_water_mixed_v6.log
...
for tag in small_Si medium_C large_water; do
  ...
  grep "^Epoch:" train_${tag}_mixed_v6.log 2>/dev/null | tail -50 | awk '{for(i=1;i<=NF;i++) if($i=="Time") {t+=$(i+1);n++}} END{printf "%.4f s/step\n", t/n}'
done
