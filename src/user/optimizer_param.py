...
class OptimizerParam(object):
    ...
    def set_optimizer(self, json_source:dict, nep_param:NepParam=None):
        ...
        if "KF" in self.opt_name.upper():
            ...
            self.start_pre_fac_force = get_parameter("start_pre_fac_force", optimizer_dict, 100.0)
            self.start_pre_fac_etot = get_parameter("start_pre_fac_etot", optimizer_dict, 1.0)
            self.start_pre_fac_ei = get_parameter("start_pre_fac_ei", optimizer_dict, 0.1)
            self.start_pre_fac_virial = get_parameter("start_pre_fac_virial", optimizer_dict, 0.1)
            self.start_pre_fac_egroup = get_parameter("start_pre_fac_egroup", optimizer_dict, 0.1)
            self.start_pre_fac_charge = get_parameter("start_pre_fac_charge", optimizer_dict, 0.1)
            self.start_pre_fac_bec = get_parameter("start_pre_fac_bec", optimizer_dict, 1.0)
            ...
            self.end_pre_fac_virial = get_parameter("end_pre_fac_virial", optimizer_dict, 0.1)
            self.end_pre_fac_egroup = get_parameter("end_pre_fac_egroup", optimizer_dict, 0.1)
