...
if [ "$SHOW_HELP" = true ]; then
  ...
  echo "  -p     可选参数：进入 MatPL-2025.3 目录执行 git pull origin 2025.3"
...
if [ "$PULL_CODE" = true ]; then
  ...
  echo "正在进入 $SOURCE_DIR 目录执行 git pull origin 2025.3 ..."
  cd "$SOURCE_DIR" || exit 1
  git pull origin 2025.3
  if [ $? -eq 0 ]; then
    echo "git pull origin 2025.3 执行成功。"
  else
...
tar --exclude="$SOURCE_DIR/.git" \
    ...
    cmp_op.sh cmp_fortran.sh cmp_lmps.sh cmp_lmps_fortran.sh
