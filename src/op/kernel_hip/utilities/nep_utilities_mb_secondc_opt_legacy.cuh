...
static __device__ __forceinline__ void
scd_accumulate_blm_rij_l4_compact(const double d12,
                                  const double x,
                                  const double y,
                                  const double z,
                                  double* blm,
                                  ...
                                  double* dblm_r)
{
  ...
  const double x12 = x * d12inv;
  const double y12 = y * d12inv;
  const double z12 = z * d12inv;
  const double x2 = x * x;
  const double y2 = y * y;
  const double z2 = z * z;
  const double xy = x * y;
  const double xz = x * z;
  const double yz = y * z;
  const double r2 = d12 * d12;
  const double x2my2 = x2 - y2;
  const double xyz = x * yz;
  const double x3 = x * x2;
  const double y3 = y * y2;
  const double z3 = z * z2;
  const double r3 = d12 * r2;
  const double x12sq = x12 * x12;
  const double y12sq = y12 * y12;
  const double z12sq = z12 * z12;
  const double x12sq_minus_y12sq = x12sq - y12sq;

  blm[0] = (35.0 * z2 - 30.0 * r2) * z2 + 3.0 * r2 * r2;
  dblm_r[0] = -60.0 * z2 * d12 + 12.0 * r3;
  dblm_x[0] = 0.0; dblm_y[0] = 0.0; dblm_z[0] = 140.0 * z3 - 60.0 * z * r2;
  blm[1] = (7.0 * z2 - 3.0 * r2) * xz;
  dblm_r[1] = -6.0 * xz * d12; dblm_x[1] = 7.0 * z3 - 3.0 * z * r2;
  dblm_y[1] = 0.0; dblm_z[1] = 21.0 * x * z2 - 3.0 * x * r2;
  blm[2] = (7.0 * z2 - 3.0 * r2) * yz;
  dblm_r[2] = -6.0 * yz * d12; dblm_x[2] = 0.0;
  dblm_y[2] = 7.0 * z3 - 3.0 * z * r2; dblm_z[2] = 21.0 * y * z2 - 3.0 * y * r2;
  blm[3] = (7.0 * z2 - r2) * x2my2;
  dblm_r[3] = 2.0 * d12 * (y2 - x2); dblm_x[3] = 14.0 * x * z2 - 2.0 * x * r2;
  dblm_y[3] = 2.0 * y * (r2 - 7.0 * z2); dblm_z[3] = 14.0 * (x2 - y2) * z;
  blm[4] = (7.0 * z2 - r2) * 2.0 * xy;
  dblm_r[4] = -4.0 * xy * d12; dblm_x[4] = 2.0 * y * (7.0 * z2 - r2);
  dblm_y[4] = 2.0 * x * (7.0 * z2 - r2); dblm_z[4] = 28.0 * xyz;
  blm[5] = (x2 - 3.0 * y2) * xz;
  dblm_r[5] = 0.0; dblm_x[5] = 3.0 * z * (x2 - y2); dblm_y[5] = -6.0 * xyz;
  dblm_z[5] = x3 - 3.0 * x * y2;
  blm[6] = (3.0 * x2 - y2) * yz;
  dblm_r[6] = 0.0; dblm_x[6] = 6.0 * xyz; dblm_y[6] = 3.0 * (x2 - y2) * z;
  dblm_z[6] = 3.0 * y * x2 - y3;
  blm[7] = x2my2 * x2my2 - 4.0 * x2 * y2;
  dblm_r[7] = 0.0; dblm_x[7] = 4.0 * x3 - 12.0 * x * y2;
  dblm_y[7] = 4.0 * y3 - 12.0 * x2 * y; dblm_z[7] = 0.0;
  blm[8] = 4.0 * x2my2 * xy;
  dblm_x[8] = 12.0 * x2 * y - 4.0 * y3;
  dblm_y[8] = 4.0 * x3 - 12.0 * x * y2; dblm_z[8] = 0.0;
}


struct ScdWeightedSView {
  const double* values;
  int offset;

  __device__ __forceinline__ double operator[](const int index) const
  {
    return values[offset + index] * C3B[offset + index];
  }
};
...
template <bool RADIAL>
static __device__ __forceinline__ double scd_get_f12_4body_direction(
  const double *fn12,
  const double *fnp12,
  const double *blm,
  const double *dblm_dir,
  const double dc0, const double dc1, const double dc2, const double dc3, const double dc4,
  const double s0, const double s1, const double s2, const double s3, const double s4,
  const double ss0, const double ss1, const double ss2, const double ss3, const double ss4,
  const double fn,
  const double fnp,
  const double rij_Lsq,
  const double rij_L2sq,
  const int k)
{
  double dir0, dir1, dir2, dir3, dir4;
  double dir_dc0, dir_dc1, dir_dc2, dir_dc3, dir_dc4;
  if constexpr (RADIAL) {
    dir0 = fnp * blm[3] + fn * dblm_dir[3];
    dir1 = fnp * blm[4];
    dir2 = fnp * blm[5];
    dir3 = fnp * blm[6];
    dir4 = fnp * blm[7];
    const double radial_scale = fnp12[k] * rij_Lsq - 2.0 * fn12[k] * rij_L2sq;
    dir_dc0 = radial_scale * blm[3] + fn12[k] * rij_Lsq * dblm_dir[3];
    dir_dc1 = radial_scale * blm[4];
    dir_dc2 = radial_scale * blm[5];
    dir_dc3 = radial_scale * blm[6];
    dir_dc4 = radial_scale * blm[7];
  } else {
    const double direction_scale = fn12[k] * rij_Lsq;
    dir0 = fn * dblm_dir[3];
    dir1 = fn * dblm_dir[4];
    dir2 = fn * dblm_dir[5];
    dir3 = fn * dblm_dir[6];
    dir4 = fn * dblm_dir[7];
    dir_dc0 = direction_scale * dblm_dir[3];
    dir_dc1 = direction_scale * dblm_dir[4];
    dir_dc2 = direction_scale * dblm_dir[5];
    dir_dc3 = direction_scale * dblm_dir[6];
    dir_dc4 = direction_scale * dblm_dir[7];
  }
  ...
  double tmp = 0.0;
    ...
    // d0
    tmp += 3.0 * C4B[0] * (
      2.0 * s0 * dc0 * dir0 + ss0 * dir_dc0);
    // d1
    tmp += C4B[1] * (
      dir_dc0 * (ss1 + ss2) + dir0 * 2.0 * (s1 * dc1 + s2 * dc2)
    );
    tmp += 2.0 * C4B[1] * (
      dc0 * (s1 * dir1 + s2 * dir2) +
      s0 * (dc1 * dir1 + s1 * dir_dc1 +
                dc2 * dir2 + s2 * dir_dc2)
    );
    // d2
    tmp += C4B[2] * (
      dir_dc0 * (ss3 + ss4) + dir0 * 2.0 * (s3 * dc3 + s4 * dc4));
    tmp += 2.0 * C4B[2] * (
      dc0 * (s3 * dir3 + s4 * dir4) +
      s0 * (dc3 * dir3 + s3 * dir_dc3 +
                dc4 * dir4 + s4 * dir_dc4)
    );
    // d3
    tmp += C4B[3] * (
      dir_dc3 * (ss2 - ss1) + dir3 * 2.0 * (s2 * dc2 - s1 * dc1));
    tmp += 2.0 * C4B[3] * (
      dc3 * (s2 * dir2 - s1 * dir1) +
        s3 * (dc2 * dir2 + s2 * dir_dc2 -
                dc1 * dir1 - s1 * dir_dc1)
    );
    // d4
    tmp += C4B[4] * (
      dir_dc1 * s2 * s4 + dir1 * dc2 * s4 + dir1 * s2 * dc4 +
      dc1 * dir2 * s4 + s1 * dir_dc2 * s4 + s1 * dir2 * dc4 +
      dc1 * s2 * dir4 + s1 * dc2 * dir4 + s1 * s2 * dir_dc4
    );
  return tmp;
}

static __device__ __forceinline__ void scd_get_f12_4body_scalars(
  const double *fn12,
  const double *fnp12,
  const double *blm,
  const double *rij_blm,
  ...
  const double *scd_r12,
  const double *dsnlm_dc,
  const double input_s0, const double input_s1, const double input_s2, const double input_s3, const double input_s4,
  const double* r12,
  const double d12inv,
  const double rij_Lsq,
  const double rij_L2sq,
  ...
  double *f12k)
{
  const int dsnlm_idx = dsnlm_start_idx + type_j * n_base_angular * NUM_OF_ABC;
  const double s0 = input_s0, s1 = input_s1, s2 = input_s2, s3 = input_s3, s4 = input_s4;
  const double ss0 = s0 * s0, ss1 = s1 * s1, ss2 = s2 * s2, ss3 = s3 * s3, ss4 = s4 * s4;

  for (int k = 0; k < n_base_angular; ++k) {
    const int dsnlm_i = dsnlm_idx + k * NUM_OF_ABC;
    const double dc0 = dsnlm_dc[dsnlm_i + 3];
    const double dc1 = dsnlm_dc[dsnlm_i + 4];
    const double dc2 = dsnlm_dc[dsnlm_i + 5];
    const double dc3 = dsnlm_dc[dsnlm_i + 6];
    const double dc4 = dsnlm_dc[dsnlm_i + 7];
    const int k_idx = k * 4;
    f12k[k] += Fp * scd_r12[0] * scd_get_f12_4body_direction<true>(
      fn12, fnp12, blm, dblm_r, dc0, dc1, dc2, dc3, dc4, s0, s1, s2, s3, s4, ss0, ss1, ss2, ss3, ss4, fn, fnp, rij_Lsq, rij_L2sq, k);
    f12k[k] += Fp * scd_r12[1] * scd_get_f12_4body_direction<false>(
      fn12, fnp12, blm, dblm_x, dc0, dc1, dc2, dc3, dc4, s0, s1, s2, s3, s4, ss0, ss1, ss2, ss3, ss4, fn, fnp, rij_Lsq, rij_L2sq, k);
    f12k[k] += Fp * scd_r12[2] * scd_get_f12_4body_direction<false>(
      fn12, fnp12, blm, dblm_y, dc0, dc1, dc2, dc3, dc4, s0, s1, s2, s3, s4, ss0, ss1, ss2, ss3, ss4, fn, fnp, rij_Lsq, rij_L2sq, k);
    f12k[k] += Fp * scd_r12[3] * scd_get_f12_4body_direction<false>(
      fn12, fnp12, blm, dblm_z, dc0, dc1, dc2, dc3, dc4, s0, s1, s2, s3, s4, ss0, ss1, ss2, ss3, ss4, fn, fnp, rij_Lsq, rij_L2sq, k);
  }
}

static __device__ __forceinline__ void scd_get_f12_4body(
  const double *fn12, const double *fnp12, const double *blm, const double *rij_blm,
  ...
  const double d12inv, const double rij_Lsq, const double rij_L2sq, const double fn, const double fnp, const double Fp,
  const int n_base_angular, const int dsnlm_start_idx, const int type_j, const int ntypes, const int n1, const int n2,
  double *f12k)
{
  scd_get_f12_4body_scalars(
    fn12, fnp12, blm, rij_blm, dblm_x, dblm_y, dblm_z, dblm_r, scd_r12, dsnlm_dc,
    s[0], s[1], s[2], s[3], s[4], r12, d12inv, rij_Lsq, rij_L2sq, fn, fnp, Fp,
...
struct ScdL4CompactView {
  const double* values;

  __device__ __forceinline__ double operator[](const int index) const
  {
    return values[index - 15];
  }
};

template <typename ArrayView, typename SView>
static __device__ __forceinline__ void scd_get_f12_4(
  const double *fn12,
  const double *fnp12,
  const ArrayView& blm,
  const ArrayView& rij_blm,
  const ArrayView& dblm_x,
  const ArrayView& dblm_y,
  const ArrayView& dblm_z,
  const ArrayView& dblm_r,
  const double *scd_r12,
  const double *dsnlm_dc,
  const SView& s,
  const double* r12,
  ...
  const bool same_type = false)
{
  ...
  const double* sum_n = sum_fxyz + n * NUM_OF_ABC;
  const ScdWeightedSView s1{sum_n, 0};
  if (same_type) {
...
template <bool INCLUDE_L4 = true, int SAME_TYPE_MODE = -1>
  ...
  const bool same_type = false)
{
  ...
  const double* sum_n = sum_fxyz + n * NUM_OF_ABC;
  const ScdWeightedSView s1{sum_n, 0};
  if (SAME_TYPE_MODE < 0 ? same_type : SAME_TYPE_MODE == 1) {
  scd_get_f12_1(fn12, fnp12,
              blm, rij_blm, dblm_x, dblm_y, dblm_z, dblm_r,
              scd_r12, dsnlm_dc, s1, r12,
              d12inv, rij_Lsq, rij_L2sq, fn, fnp, Fp[n*lmax_3],
  ...
  rij_Lsq = rij_L2sq;
  rij_L2sq = rij_L2sq * d12inv;
  const ScdWeightedSView s2{sum_n, 3};

  if (SAME_TYPE_MODE < 0 ? same_type : SAME_TYPE_MODE == 1) {
    scd_get_f12_4body_scalars(fn12, fnp12,
      blm, rij_blm, dblm_x, dblm_y, dblm_z, dblm_r,
      scd_r12, dsnlm_dc, sum_n[3], sum_n[4], sum_n[5], sum_n[6], sum_n[7], r12,
  ...
  rij_Lsq = rij_L2sq;
  rij_L2sq = rij_L2sq * d12inv;
  const ScdWeightedSView s3{sum_n, 8};
  if (SAME_TYPE_MODE < 0 ? same_type : SAME_TYPE_MODE == 1) {
  scd_get_f12_3(fn12, fnp12,
                blm, rij_blm, dblm_x, dblm_y, dblm_z, dblm_r,
                scd_r12, dsnlm_dc, s3, r12,
  ...
  rij_Lsq = rij_L2sq;
  rij_L2sq = rij_L2sq * d12inv;
  const ScdWeightedSView s4{sum_n, 15};
  if (SAME_TYPE_MODE < 0 ? same_type : SAME_TYPE_MODE == 1) {
  scd_get_f12_4(fn12, fnp12,
                blm, rij_blm, dblm_x, dblm_y, dblm_z, dblm_r,
                scd_r12, dsnlm_dc, s4, r12,
...
template <int SAME_TYPE_MODE = -1>
static __device__ __forceinline__ void scd_accumulate_f12_l4_compact(
  const int n, const double d12, const double* r12, double fn, double fnp,
  const double* Fp, const double* dsnlm_dc, const double* sum_fxyz,
  double* blm, double* rij_blm, double* dblm_x, double* dblm_y,
  double* dblm_z, double* dblm_r, double* f12k, double* scd_r12,
  double* fn12, double* fnp12, const int type_j, const int ntypes,
  const int lmax_3, const int n_base_angular, const int dsnlm_start_idx,
  const int n1, const int n2, const bool same_type)
{
  const double d12inv = 1.0 / d12;
  double rij_Lsq = d12inv;
  double rij_L2sq = d12inv * d12inv;
  ...
  const ScdWeightedSView s4{sum_fxyz + n * NUM_OF_ABC, 15};

  const ScdL4CompactView compact_blm{blm};
  const ScdL4CompactView compact_rij_blm{rij_blm};
  const ScdL4CompactView compact_dblm_x{dblm_x};
  const ScdL4CompactView compact_dblm_y{dblm_y};
  const ScdL4CompactView compact_dblm_z{dblm_z};
  const ScdL4CompactView compact_dblm_r{dblm_r};
  if (SAME_TYPE_MODE < 0 ? same_type : SAME_TYPE_MODE == 1) {
    scd_get_f12_4(fn12, fnp12, compact_blm, compact_rij_blm,
      compact_dblm_x, compact_dblm_y, compact_dblm_z, compact_dblm_r,
      scd_r12, dsnlm_dc, s4, r12, d12inv, rij_Lsq, rij_L2sq, fn, fnp,
      Fp[n * lmax_3 + 3], n_base_angular, dsnlm_start_idx, type_j,
      ntypes, n1, n2, f12k);
  } else {
    scd_get_f12_4_J(fn12, fnp12, compact_blm, compact_rij_blm,
      compact_dblm_x, compact_dblm_y, compact_dblm_z, compact_dblm_r,
      scd_r12, dsnlm_dc, s4, r12, d12inv, rij_Lsq, rij_L2sq, fn, fnp,
      Fp[n * lmax_3 + 3], n_base_angular, dsnlm_start_idx, type_j,
      ntypes, n1, n2, f12k);
...
template <int SAME_TYPE_MODE = -1>
static __device__ __forceinline__ void scd_accumulate_f12_5body_only(
  const int n, const double d12, const double* r12, const double fn,
  const double fnp, const double* Fp, const double* dsnlm_dc,
  const double* sum_fxyz, const double* blm, const double* rij_blm,
  const double* dblm_x, const double* dblm_y, const double* dblm_z,
  const double* dblm_r, double* f12k, const double* scd_r12,
  const double* fn12, const double* fnp12, const int type_j,
  const int ntypes, const int lmax_3, const int n_max_angular,
  const int n_base_angular, const int dsnlm_start_idx, const int n1,
  const int n2, const bool same_type)
{
  const double d12inv = 1.0 / d12;
  const double s1[3] = {
      sum_fxyz[n * NUM_OF_ABC], sum_fxyz[n * NUM_OF_ABC + 1],
      sum_fxyz[n * NUM_OF_ABC + 2]};
  if (SAME_TYPE_MODE < 0 ? same_type : SAME_TYPE_MODE == 1) {
    scd_get_f12_5body(
      fn12, fnp12, blm, rij_blm, dblm_x, dblm_y, dblm_z, dblm_r,
      scd_r12, dsnlm_dc, s1, r12, d12inv, d12inv, d12inv * d12inv,
      fn * d12inv, fnp * d12inv - fn * d12inv * d12inv,
      Fp[n_max_angular * lmax_3 + n_max_angular + n], n_base_angular,
      dsnlm_start_idx, type_j, ntypes, n1, n2, f12k);
  } else {
    scd_get_f12_5body_J(
      fn12, fnp12, blm, rij_blm, dblm_x, dblm_y, dblm_z, dblm_r,
      scd_r12, dsnlm_dc, s1, r12, d12inv, d12inv, d12inv * d12inv,
