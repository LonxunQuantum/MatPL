...
每个模块导出固定名称的四个 fitting kernel。PyTorch launcher 只负责输入检查和
在当前 CUDA stream 上启动专用 kernel，不再包含第二套 AOT fitting 实现。NVRTC
编译或模块加载失败时直接报错。输入、输出和参数张量的布局不发生变化。
...
CUDA fused fitting 始终使用 JIT，不再设置启用开关，也不存在静默回退路径。JIT
只对 CUDA 构建生效，CPU 和 HIP 路径保持不变。
...
不支持的维度、NVRTC 缺失、编译失败、缓存目录不可写、module 加载失败和 kernel
启动失败均直接抛出。单一执行路径避免隐藏环境问题，也避免输出已经部分写入后尝试
另一套实现。
...
1. 缓存键稳定，当前 SM 可以生成有效缓存文件；
2. 第二个进程可以复用已有缓存；
3. 多进程并发准备只产生一个有效缓存文件；
4. 缓存路径无效时直接报错，损坏的 CUBIN 可以安全重建；
5. 边界尺寸和非 16 对齐尺寸下，JIT 的输出、feature 一阶导、feature 反向梯度、
   参数梯度和 Adam 更新与独立 PyTorch 数学参考一致；
6. `Q=2` charge 模式、非默认 stream、DDP，以及 P100、V100、3080 Ti、3090、4090
   对应架构的编译均受支持；
7. OMat24 基准将编译/加载耗时与稳态 step 耗时分开统计，并记录显存峰值。
